...
describe( 'Apigeon: /core/libs/errorClass.js', function () {
    ...
    it( 'should contain a method getMessage() which returns the message associate with the code (provided in the constructor)', function ( done ) {
        ...
        expect( e.getMessage() ).to.have.a.property( 'error' );
        expect( e.getMessage().error ).to.have.any.keys( 'code', 100 );
        expect( e.getMessage().error ).to.have.any.keys( 'message', 'test' );
        done();
    } );
    ...
    it( 'should output the appropriate message for each code.', function ( done ) {
        var e;
        e = new Victim( 403 );
        expect( e.getMessage().error ).to.have.any.keys( 'message', 'Access denied.' );
        e = new Victim( 404 );
        expect( e.getMessage().error ).to.have.any.keys( 'message', 'Page not found.' );
        e = new Victim( 405 );
        expect( e.getMessage().error ).to.have.any.keys( 'message', 'Method not allowed.' );
        e = new Victim( 500 );
        expect( e.getMessage().error ).to.have.any.keys( 'message', 'There was an error.' );
        e = new Victim( 503 );
        expect( e.getMessage().error ).to.have.any.keys( 'message', 'Not Implemented.' );
        done();
    } );
